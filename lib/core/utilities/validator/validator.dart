...
class PassWordNotSameEmail extends Validator {
  PassWordNotSameEmail(String errorText, this.email) : super(errorText, []);
  final String email;
  @override
  String? validate(String? value) {
    return (value != null && value != '' && value == email) ? errorText : null;
  }
}
...
enum TypeValidate { none, alphabetic, numeric, alphaNumeric }

class EmailValidatorModel {
  static int _index = 0;
  static const String _atomCharacters = "!#\$%&'*+-/=?^_`{|}~";
  static TypeValidate _domainType = TypeValidate.none;
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = TypeValidate.alphabetic;
        return true;
      }

      if (_isDigit(c)) {
        _domainType = TypeValidate.numeric;
        return true;
      }
  ...
  static bool _skipDomain(
    ...
    if (_index < text.length && text[_index] == '.') {
      do {
        ...
        var arrSub = arr.split('.');

        if (arrSub.last.length < 2) {
          return false;
        }
    ...
    if (_domainType == TypeValidate.numeric) {
      return false;
    }
  ...
  static bool validate(
    ...
    if (_index + 1 >= email.length || email[_index++] != '@') {
      return false;
    }
